...
function ReportByPeriod () {
    ...
    const period = async () => {
        ...
        setLoading(true);
        try {
            const response = await api.get("/api/reports/period", { params: { startDate, endDate } });
    ...
    return (
        <div className="report-page">
            ...
            {loading ? (
                ...
                <div className="empty-state">Selecciona un rango de fechas</div>
            ) : report.length === 0 ? (
                <div className="empty-state">No se encontraron paquetes en este período</div>
            ) : (
                <div className="report-grid">
                    {report.map((item, index) => (
                        <div key={item.reservationId} className="report-card">
                            <div className="report-card__body">
                                <h3 className="report-card__name">{item.packageName}</h3>
                                <p className="report-card__destination">📍 {item.destination}</p>
                                <p className="report-card__destination">
                                    📅 {new Date(item.createdAt).toLocaleString('es-CL', {
                                        day: '2-digit', month: '2-digit', year: 'numeric',
                                        hour: '2-digit', minute: '2-digit'
                                    })}
                                </p>
                                <p className="report-card__destination">👤 {item.clientName} — {item.clientEmail}</p>
                                <div className="report-card__stats">
                                    <span>👥 {item.passengerCount} pasajeros</span>
                                    <span>💰 ${item.totalPrice.toLocaleString('en-US')}</span>
                                    {item.discountAmount > 0 && (
                                        <span>🏷️ Descuento: ${item.discountAmount.toLocaleString('en-US')}</span>
                                    )}
                                </div>
                            </div>
                        </div>
